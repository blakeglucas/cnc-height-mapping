...
export class SocketService {
  ...
  private readonly _serialPorts = new BehaviorSubject<string[]>(DEFAULT_EMPTY_ARRAY);
  readonly serialPorts$ = this._serialPorts.asObservable();

  private readonly _activeMachine = new BehaviorSubject<string>(DEFAULT_EMPTY_STRING);
  readonly activeMachine$ = this._activeMachine.asObservable();

  private readonly _machinePosition = new BehaviorSubject<number[]>(DEFAULT_EMPTY_ARRAY);
  ...
  createNewSocket(ip: string, port: number) {
    ...
    this._currentSocket.on(EventTypes.GetMachinePosition, (isOk: string, pos: number[] | null) => {
      console.log(isOk)
      if (pos) {
